...
python manage.py --key-pair=next_key_1 --identity-file=/Users/kevinjamieson/aws_keys/next_key_1.pem  launch kevin_test

"""

EC2_SRC_PATH = '/usr/local/web/'
LOCAL_SRC_PATH = './../'
...
def parse_args():
    parser = OptionParser(
        ...
        + "<action> can be: launch, destroy, login, stop, start, get-master, reboot-slaves, rsync, docker_up, docker_login")
    parser.add_option(
...
def setup_cluster(conn, master_nodes, slave_nodes, opts, deploy_ssh_key):
    ...
    ssh(master, opts, "sudo rm -rf " + EC2_SRC_PATH)
    ssh(master, opts, "sudo mkdir " + EC2_SRC_PATH)
    ssh(master, opts, "sudo chmod 777 " + EC2_SRC_PATH)
    command = [
        'rsync', '-rv',
        '-e', stringify_command(ssh_command(opts)),
        "%s/" % tmp_dir,
        "%s@%s:%s/" % (opts.user, master, EC2_SRC_PATH)
    ]
    subprocess.check_call(command)
    # Remove the temp directory we created above
    shutil.rmtree(tmp_dir)

    ssh(master, opts, "sudo chmod 777 " + EC2_SRC_PATH + '/' + 'setup.sh')
    ssh(master, opts, 'sudo su -c ' + EC2_SRC_PATH + '/' + 'setup.sh')
    print "Done!"


    print "Start rsync of local source code up"
    rsync_dir(LOCAL_SRC_PATH, EC2_SRC_PATH, opts, master)
    print "Done!"
...
def docker_up(opts, master_nodes, slave_nodes):
    ...
    ssh(master, opts, "sudo chmod 777 " + EC2_SRC_PATH + '/' + 'docker_up.sh')
    ssh(master, opts, 'sudo su -c ' + EC2_SRC_PATH + '/' + 'docker_up.sh')
...
def rsync_docker_config(opts, master_nodes, slave_nodes):
    ...
    with open('./templates/docker_login.sh') as src:
        with open(tmp_dir+'/docker_login.sh', "w") as dest:
            ...
            text = text.replace("{{ environment_variables }}",env_vars)
            text = text.replace("{{ EC2_SRC_PATH }}",EC2_SRC_PATH)
            dest.write(text)
            dest.close()
...
def docker_login(opts, master_nodes, slave_nodes):
    ...
    ssh(master, opts, "sudo chmod 777 " + EC2_SRC_PATH + '/' + 'docker_login.sh')
    # ssh(master, opts, 'sudo su -c' + EC2_SRC_PATH + '/' + 'docker_login.sh')
    command = ' sudo su -c' + EC2_SRC_PATH + '/' + 'docker_login.sh'
    ...
    print 
    print 'sudo su'
    print EC2_SRC_PATH + '/' + 'docker_login.sh'
    print
    print 60*'#'
    print 60*'#'
