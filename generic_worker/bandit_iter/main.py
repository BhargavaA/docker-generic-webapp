...
def run_trials(num_arms,train_size,num_iters,UID='',params=None):
    ...
    for i in range(n):
        ...
        boto_conn.write_to_s3(local_filename_path=UID+"_jobs.txt",s3_path='kgjamieson-general-compute/hyperband_data_bandit_iter_round2/'+filename)
        ########################################
...
num_arms = int(B/num_iters)
while num_iters>=min_iter:

    if num_arms>2:
        meta_arms.append([num_arms,max_train_size,num_iters])

    num_iters = int(num_iters/2)
    num_arms = int(B/num_iters)

print meta_arms
meta_arms = sorted(meta_arms,key= lambda student: student[2])
print meta_arms
...
while True:
    ...
    while dt<stop_time:
        ...
        if min(meta_T)==0:
            idx = numpy.argmin(meta_T)
            print "init: "+str(idx)
        else:
            if numpy.random.rand()<0.25:
                idx = numpy.random.choice(meta_n)
                print "random: "+str(idx)
            else:
                mu_hat = [ meta_sum[i]/max(1.,meta_T[i]) for i in range(meta_n) ]
                idx = numpy.argmax(mu_hat)
                print "greedy: "+str(idx)
        # idx = numpy.random.choice(meta_n)
