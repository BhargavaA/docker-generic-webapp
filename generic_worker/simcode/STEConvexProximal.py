...
def run(n, d, plot=False, verbose=False):
    ...
    Mhat, emp_loss_train = computeEmbedding(n,d,Strain,l,
...
def computeEmbedding(n,d,S,l,num_random_restarts=0, max_num_passes_SGD=16,max_iter_GD=50,max_norm=1,epsilon=0.01,verbose=False):
    """
    ...
    for restart in range(num_random_restarts):
        ...
        M_new,emp_loss_new,log_loss_new,acc_new = computeEmbeddingWithGD(M,S,l,
...
def proximal(M, t):
    '''
    Proximal for psd+nuclear norm constraint
    See Lemma 1 of 
...
def computeEmbeddingWithGD(M,S,l,max_iters=50,max_norm=1.,epsilon=0.001,c1=.00001,rho=0.5,verbose=False):
    """
    ...
    while t < max_iters:
        ...
        alpha = 1.3*alpha
        M_k = proximal(M-alpha*G, alpha*l)
