...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=5,
   ...
   """
    ...
    if n_train==None:
        n_train = float('inf')
    n_train_batches = min(train_set_x.get_value(borrow=True).shape[0],n_train) / batch_size
    ...
    validation_frequency = min(n_train_batches, patience / 2)   # usually 100
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                #     )
                # )
                error_progress.append(this_validation_loss)
    ...
    if verbose:  print(('Optimization complete. Best validation score of %f %% '
...
if __name__ == '__main__':
    ...
    for k in range(max_k):
        train_dataset_proportion = 1.
        learning_rate = 10**rng.uniform(-3,-1) # default = 0.01
        L2_reg = 10**rng.uniform(-6,-1) # default 0.0001
        n_hidden = int(10**rng.uniform(1,3)) # default 500

        if iter_opt:
            error_progress = test_mlp(learning_rate=learning_rate, L1_reg=0.00, L2_reg=L2_reg, n_epochs=40,
                 dataset='mnist.pkl.gz', batch_size=20, n_hidden=n_hidden)
            results[(learning_rate,L2_reg,n_hidden)] = copy.deepcopy(error_progress)
        else:
            losses = []
            times = []
            batch_sizes = []
            ...
            for ell in range(max_ell+1):
                n_train = int( max_train*2**(ell-max_ell) )

                print
                print "starting %d/%d, %d/%d: %s" % (k,max_k,ell,max_ell,str((learning_rate,L2_reg,n_hidden,n_train)) )

                ts = time.time()
                error_progress = test_mlp(learning_rate=learning_rate, L1_reg=0.00, L2_reg=L2_reg, n_epochs=40,
                     dataset='mnist.pkl.gz', batch_size=20, n_hidden=n_hidden, n_train=n_train)
                te = time.time()

                batch_sizes.append(n_train)
                times.append(te-ts)
                losses.append(error_progress[-1])

            batch_progress = copy.deepcopy(batch_sizes)
            time_progress = copy.deepcopy(times)
            error_progress = copy.deepcopy(losses)
            
            results[(learning_rate,L2_reg,n_hidden,n_train)] = [batch_progress,time_progress,error_progress]
            print "saved: %s" % str((learning_rate,L2_reg,n_hidden))
