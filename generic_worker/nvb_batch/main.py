...
def run_trials(num_arms,train_size,num_iters,UID='',params=None):
    ...
    for i in range(n):
        ...
        boto_conn.write_to_s3(local_filename_path=UID+"_jobs.txt",s3_path='kgjamieson-general-compute/hyperband_data_nvb_batch_round3/'+filename)
        ########################################
    ...
    while dt<stop_time:
        ...
        while num_pulls>=min_train_size and dt<stop_time:
            ...
            if num_arms>2:
                ...
                pre_error,this_hyperparameters,this_dt = run_trials(num_arms=num_arms,train_size=num_pulls,num_iters=max_iter,UID=UID)
                dt += this_dt
