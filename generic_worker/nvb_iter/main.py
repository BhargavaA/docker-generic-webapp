...
def run_trials(num_arms,train_size,num_iters,UID='',params=None):
    ...
    for i in range(n):
        ...
        boto_conn.write_to_s3(local_filename_path=UID+"_jobs.txt",s3_path='kgjamieson-general-compute/hyperband_data_nvb_iter/'+filename)
        ########################################
...
while True:
    ...
    while dt<stop_time:
        ...
        num_pulls = int(max_iter)
        num_arms = int(B/num_pulls)
        while num_pulls>=min_iter and dt<stop_time:
